import { v4 as uuidv4 } from 'uuid';
import BadRequestException from '../../src/Shared/Domain/Exception/BadRequestException';
import InternalException from '../../src/Shared/Domain/Exception/InternalException';
import NotFoundException from '../../src/Shared/Domain/Exception/NotFoundException';
import Base64Image from '../../src/Shared/Domain/ValueObject/Base64Image';
import Base64Zip from '../../src/Shared/Domain/ValueObject/Base64Zip';
import CommaTags from '../../src/Shared/Domain/ValueObject/CommaTags';
import Description from '../../src/Shared/Domain/ValueObject/Description';
import Email from '../../src/Shared/Domain/ValueObject/Email';
import FileName from '../../src/Shared/Domain/ValueObject/FileName';
import FilePath from '../../src/Shared/Domain/ValueObject/FilePath';
import Id from '../../src/Shared/Domain/ValueObject/Id';
import LimitedString from '../../src/Shared/Domain/ValueObject/LimitedString';
import Name from '../../src/Shared/Domain/ValueObject/Name';
import Password from '../../src/Shared/Domain/ValueObject/Password';
import Poster from '../../src/Shared/Domain/ValueObject/Poster';
import Tags from '../../src/Shared/Domain/ValueObject/Tags';
import UIntId from '../../src/Shared/Domain/ValueObject/UIntId';
import Url from '../../src/Shared/Domain/ValueObject/Url';
import Youtube from '../../src/Shared/Domain/ValueObject/Youtube';
import Namespace from '../../src/Shared/Domain/ValueObject/Namespace';
import SecureString from '../../src/Shared/Domain/ValueObject/SecureString';
import OptionalLimitedString from '../../src/Shared/Domain/ValueObject/OptionalLimitedString';

export class SharedMother {
    public static SECURE_STRING = 'Hola Mundo';
    public static TOKEN_PAYLOAD = {
        anykey: 'any random value 9d9218b7-95ce-42ec-97a7-fe4b8038c8ad',
        iat: 1701187971,
        exp: 2565101571,
    };
    public static TOKEN_PAYLOAD_JSON_BASE64 =
        'ewogICAgImFueWtleSI6ICJhbnkgcmFuZG9tIHZhbHVlIDlkOTIxOGI3LTk1Y2UtNDJlYy05N2E3LWZlNGI4MDM4YzhhZCIsCiAgICAiaWF0IjogMTcwMTE4Nzk3MSwKICAgICJleHAiOiAyNTY1MTAxNTcxCn0=';

    public static TOKEN_CREATED: string =
        'eyJhbnlrZXkiOiJhbnkgcmFuZG9tIHZhbHVlIDlkOTIxOGI3LTk1Y2UtNDJlYy05N2E3LWZlNGI4MDM4YzhhZCIsImlhdCI6MTcwMTE4Nzk3MSwiZXhwIjoyNTY1MTAxNTcxLCJhbGciOiJIUzI1NiJ9.eyJhbnlrZXkiOiJhbnkgcmFuZG9tIHZhbHVlIDlkOTIxOGI3LTk1Y2UtNDJlYy05N2E3LWZlNGI4MDM4YzhhZCIsImlhdCI6MTcwMTE4Nzk3MSwiZXhwIjoyNTY1MTAxNTcxfQ.3NOkx31GNFJox1uISy3BBkEY65HWpikfJFWzGCw9ogk';
    public static NAMESPACE: string = 'my.company.and.other.things';
    public static VALID_TOKEN: string = 'my.token.is any string';
    public static VALID_UINT_ID: number = 1;
    public static YOUTUBE_URL: string = 'https://www.youtube.com/watch?v=U0m9lHX8x_4';
    public static YOUTUBE_URL_2: string = 'https://youtu.be/U0m9lHX8x_4?si=NliLO2zgZTXJme3n';
    public static YOUTUBE_URL_3: string = 'https://www.youtube.com/embed/U0m9lHX8x_4?si=NliLO2zgZTXJme3n';
    public static YOUTUBE_URL_4: string =
        'https://www.youtube.com/watch?v=4Hf5rERu5AI&list=PL-1zlyAOA1_6hu25zBMQEa4oB7GwYKYA6';
    public static NAME_VALUE: string = 'a'.repeat(LimitedString.MINIMUM_LENGTH);
    public static TAGS_VALUE: string[] = ['a'.repeat(Tags.MINIMUM_TAG_LENGTH), 'b'.repeat(Tags.MINIMUM_TAG_LENGTH)];
    public static COMMA_TAGS_VALUE: string = SharedMother.TAGS_VALUE.join(',');
    public static VALID_NAME: string = 'a'.repeat(Name.MINIMUM_TAG_LENGTH);
    public static VALID_POSTER: string = 'https://i.ytimg.com/vi/ZvXXr9NkNkw/mqdefault.jpg';
    public static VALID_EMAIL: string = 'hola@hola.com';
    public static VALID_PASSWORD: string = 'anypassword';
    public static HELLO_WORLD_LITERAL: string = 'hello world';
    public static HELLO_WORLD_HASHED_LENGTH: number = SharedMother.HELLO_WORLD_LITERAL.length;
    public static VALID_ID: string = uuidv4();
    public static VALID_ID_2: string = uuidv4();
    public static VALID_ID_3: string = uuidv4();
    public static VALID_URL: string = 'https://www.youtube.com/watch?v=U0m9lHX8x_4';
    public static BASE64_PNG_IMAGE: string =
        '';
    public static BASE64_SVG_IMAGE: string =
        'PHN2ZyB3aWR0aD0iMSIgaGVpZ2h0PSIxIiB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciPjxyZWN0IHdpZHRoPSIxMDAlIiBoZWlnaHQ9IjEwMCUiIGZpbGw9InJlZCIvPjwvc3ZnPg==';
    public static BASE64_TEXT: string = 'bWFya2l0b3MuZXM=';
    public static BASE64_MARKDOWN_TEXT: string =
        '';
    public static BASE64_ZIP: string =
        '';

    public static Url(value?: string): Url {
        return new Url(value !== undefined ? value : SharedMother.VALID_URL);
    }

    public static SecureString(value?: string): SecureString {
        return new SecureString(value !== undefined ? value : SharedMother.SECURE_STRING);
    }


    public static Namespace(value?: string): Namespace {
        return new Namespace(value !== undefined ? value : SharedMother.NAMESPACE);
    }


    public static Poster(value?: string): Poster {
        return new Poster(value !== undefined ? value : SharedMother.VALID_POSTER);
    }

    public static FilePath(value?: string): FilePath {
        return new FilePath(value !== undefined ? value : (__filename as string));
    }

    public static FileName(value?: string): FileName {
        return new FileName(value !== undefined ? value : (__filename.split('/').pop() as string));
    }

    public static Youtube(value?: string): Youtube {
        return new Youtube(value !== undefined ? value : SharedMother.YOUTUBE_URL);
    }

    public static Base64Zip(value?: string): Base64Zip {
        return new Base64Zip(value !== undefined ? value : SharedMother.BASE64_ZIP);
    }

    public static Base64Image(value?: string): Base64Image {
        return new Base64Image(value !== undefined ? value : SharedMother.BASE64_PNG_IMAGE);
    }

    static Email(value?: string): Email {
        return new Email(value !== undefined ? value : SharedMother.VALID_EMAIL);
    }
    public static Description(value?: string): Description {
        return new Description(value !== undefined ? value : SharedMother.BASE64_TEXT);
    }

    static Password(value?: string): Password {
        return new Password(value !== undefined ? value : SharedMother.VALID_PASSWORD);
    }

    public static Id(value?: string): Id {
        return new Id(value !== undefined ? value : SharedMother.VALID_ID);
    }

    public static UIntId(value?: number): UIntId {
        return new UIntId(value !== undefined ? value : SharedMother.VALID_UINT_ID);
    }

    public static CommaTags(value?: string): CommaTags {
        return new CommaTags(value !== undefined ? value : SharedMother.COMMA_TAGS_VALUE);
    }

    public static Tags(value?: string[]): Tags {
        return new Tags(value !== undefined ? value : SharedMother.TAGS_VALUE);
    }

    public static Name(value?: string): Name {
        return new Name(value !== undefined ? value : SharedMother.BASE64_TEXT);
    }

    public static OptionalLimitedString(value?: string): OptionalLimitedString {
        return new OptionalLimitedString(value !== undefined ? value : 'a'.repeat(OptionalLimitedString.MINIMUM_LENGTH));
    }

    public static LimitedString(value?: string): LimitedString {
        return new LimitedString(value !== undefined ? value : 'a'.repeat(LimitedString.MINIMUM_LENGTH));
    }

    public static BadRequestException(): BadRequestException {
        return new BadRequestException();
    }

    public static NotFoundException(): NotFoundException {
        return new NotFoundException();
    }
    public static InternalException(): InternalException {
        return new InternalException();
    }
}
